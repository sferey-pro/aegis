...
- **`docs/TESTS.md` § 5 recense les 22 écarts** entre comportement réel et `CONTEXT.md`, groupés par nature (perte de données, fonctionnalité inatteignable, angle mort, contrat d'API incohérent). Chacun est épinglé par un test : la régression involontaire est bloquée, mais **ils ne sont pas corrigés**. Consultez cette liste avant de conclure qu'un comportement surprenant est un bug neuf.
