...
bun run test:ui     # 343 tests composants — happy-dom actif
bun run test:api    # 714 tests fonctionnels — AEGIS_TEST_NO_DOM=1
...
**1057 tests, colocalisés** : chaque fichier de code porte son test à côté de lui, nommé `*.test.ts(x)`. Référence complète dans `docs/TESTING.md` (comment on teste) et `docs/TESTS.md` (ce qui est couvert).
...
- **`docs/ISSUE.md` est la liste unique des défauts connus**, groupée par priorité et revérifiée dans le code le 21/08/2026. Ses 26 entrées marquées 🧪 sont **épinglées par un test** qui affirme le comportement défectueux : la régression involontaire est bloquée, mais le défaut n'est pas corrigé. Consultez cette liste avant de conclure qu'un comportement surprenant est un bug neuf. Corriger une entrée 🧪 implique de **retourner son test d'abord**, de vérifier qu'il échoue, puis de corriger le code — `docs/TESTS.md` § 5 donne la correspondance défaut → fichier de test.
