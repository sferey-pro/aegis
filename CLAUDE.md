...
bun run test:ui     # 363 tests composants — happy-dom actif
bun run test:api    # 805 tests fonctionnels — AEGIS_TEST_NO_DOM=1
...
**1168 tests, colocalisés** : chaque fichier de code porte son test à côté de lui, nommé `*.test.ts(x)`. Référence complète dans `docs/TESTING.md` (comment on teste) et `docs/TESTS.md` (ce qui est couvert).
