...
export const projectSourceTypeSchema = z.enum(["local", "ingest", "remote"]).default("local");
...
export const projectBodySchema = z.object({
	...
	if (data.source_type === "local" && !data.path) {
		ctx.addIssue({ code: "custom", message: "Chemin requis", path: ["path"] });
	} else if (data.source_type === "remote") {
		if (!data.remote_url) {
			ctx.addIssue({ code: "custom", message: "URL distante requise", path: ["remote_url"] });
		} else {
			try {
				new URL(data.remote_url);
			} catch {
				ctx.addIssue({ code: "custom", message: "URL invalide", path: ["remote_url"] });
			}
		}
	}
});
