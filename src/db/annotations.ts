...
export type AnnotationStatus = "pending" | "confirmed" | "not_affected" | "ignored";
...
export function upsertAnnotation(cve: string, projectId: number, data: { status?: AnnotationStatus, note?: string, fixedIn?: string | null }): Annotation {
  const db = getDb();
  
  const current = db.query(`SELECT * FROM annotations WHERE cve = ? AND project_id = ?`).get(cve, projectId) as any;
...
export function setAnnotationFix(cve: string, projectId: number, fixedIn: string | null): Annotation {
