...
export function createReport(data: { projects_audited: number; total_vulnerabilities: number; counts: RunCounts }): Report {
  const db = getDb();
  const stmt = db.prepare(`
    INSERT INTO reports (projects_audited, total_vulnerabilities, counts)
    VALUES ($projects, $total, $counts)
    RETURNING *
  `);
