...
export function saveTicket(projectId: number, pkg: string, url: string, cves: string[]) {
  const db = getDb();
  db.query(`
...
export function deleteTicket(projectId: number, pkg: string) {
  const db = getDb();
  db.query(`DELETE FROM tickets WHERE project_id = ? AND package = ?`).run(projectId, pkg);
}
