...
describe("Aggregator: CVE", () => {
  ...
  test("buildCveGroups ignores projects without runs or with errors", () => {
    const p1 = createProject({ name: "P1", path: "/1", type: "node", tool: "npm" });
    const p2 = createProject({ name: "P2", path: "/2", type: "node", tool: "npm" });
    
    // P1 run is error
    addRun({ project_id: p1.id, status: "error", total: 0, counts: { critical:0, high:0, moderate:0, low:0, info:0, unknown:0 }, vulnerabilities: [], duration_ms: 100 });
    // P2 has no runs
  ...
  test("buildCveGroups correctly aggregates and overrides fixedIn", () => {
    const p = createProject({ name: "MyProj", path: "/path", type: "node", tool: "npm" });
    ...
    addRun({
      ...
      duration_ms: 100,
      vulnerabilities: [
        { package: "lodash", title: "Pollution", severity: "high", cve: "CVE-123", link: null, versionRange: null, fixedIn: "1.0.0" },
        { package: "react", title: "XSS", severity: "critical", cve: null, link: null, versionRange: null }
      ]
    });
  ...
  test("intra-project deduplication keeps worst severity", () => {
    ...
    addRun({
      ...
      duration_ms: 100,
      vulnerabilities: [
        { package: "lodash", title: "Pollution", severity: "low", cve: "CVE-123", link: null, versionRange: null },
        { package: "lodash", title: "Pollution", severity: "high", cve: "CVE-123", link: null, versionRange: null }
      ]
    });
