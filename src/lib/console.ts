...
export function emitConsoleStart(event: Omit<ConsoleEvent, "id" | "project" | "phase">): number {
  const ctx = projectContext.getStore();
  const id = nextEventId++;
  const fullEvent: ConsoleEvent = { ...event, phase: "start", id, project: ctx?.project };
  broadcast(fullEvent);
  return id;
}
...
export function removeConsoleClient(controller: ReadableStreamDefaultController) {
